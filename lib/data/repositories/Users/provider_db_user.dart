...
import 'package:social_shop_app/data/base/api_error.dart';
...
class UserProviderDB implements UserProviderContract {
  ...
  @override
  Future<User> createUser(String username, String password, String emailAddress) async {
  ...
  Map<String, dynamic> convertItemToStorageMap(User item) {
    ...
    values['value'] = json.encode(item.toJson(full: true));
  ...
  User convertRecordToItem({dynamic record, Map<String, dynamic> values}) {
    try {
      values ??= record.value;
      final User item = User.clone().fromJson(json.decode(values['value']));
      return item;
    } catch (e) {
      return null;
    }
